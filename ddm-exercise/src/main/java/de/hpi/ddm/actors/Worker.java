...
public class Worker extends AbstractLoggingActor {
    ...
    private void handle(PasswordMessage message) {
        ...
        // this.largeMessageProxy.tell(new LargeMessageProxy.LargeMessage<>(new Master.PasswordResultMessage(this.password,this.id), this.sender()), this.self());
    }
    ...
    private void calculate(int currentIndex) {
        ...
        if (calculationRound(currentIndex)) {
            ...
            this.master.tell(new HintResultMessage(this.data, this.id), this.self());
            // this.largeMessageProxy.tell(new LargeMessageProxy.LargeMessage<>(new HintResultMessage(this.data,this.id), this.master), this.self());
            this.idle = true;
        } else {
